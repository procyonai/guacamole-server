...
int watch_procyon_hosts() {
    ...
    // Add watch on the directory (change "your_directory" to the directory you want to watch)
    wd = inotify_add_watch(fd, "your_directory", IN_MODIFY | IN_CREATE | IN_DELETE);
    if (wd == -1) {
    ...
    printf("Watching directory: your_directory\n");
